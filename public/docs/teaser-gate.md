...
The work itself should do the persuading, with a blurred, inert sample behind the invitation.

## Not for

Anything confidential. Unlike the other gates, teaser content stays in the DOM while locked.
...
npx shadcn@latest add @knock-codes/teaser-gate
```

Direct URL:

`npx shadcn@latest add https://knock.codes/r/teaser-gate.json`

Requires React 18 or 19 and Tailwind CSS 4. No other runtime dependencies.

[Live demo](https://knock.codes/playground/teaser-gate) · [Registry JSON](https://knock.codes/r/teaser-gate.json) · [Source](https://knock.codes/source/teaser-gate.tsx)
...
| heading | `ReactNode` | no | 'A closer look awaits.' | Overrides the default heading. |
| description | `ReactNode` | no | 'Your private preview is just one invitation away.' | Overrides the default description. |
...
- labels.previewAccess: `'Preview access'`
...
Installs alongside: code-field, recipient-line, expiry-pill, request-access, cooldown-notice, blur-veil.

## Accessibility

Native forms and controls, polite error and cooldown announcements, visible focus, focus transfer after unlock. This block is a region, not a modal. Teaser content is inert while locked but remains in the DOM.

## Agent instructions and anti-hallucination contract

Install and wire Teaser gate (TeaserGate) with `npx shadcn@latest registry add @knock-codes` then `npx shadcn@latest add @knock-codes/teaser-gate`, or from https://knock.codes/r/teaser-gate.json. Read https://knock.codes/docs/teaser-gate.md before editing.
