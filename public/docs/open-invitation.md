...
The invitation is the access: a named recipient and a continue action, with no code to type.

## Not for

When a typed code is the proof. Use Client preview gate or Quick gate.
...
npx shadcn@latest add @knock-codes/open-invitation
```

Direct URL:

`npx shadcn@latest add https://knock.codes/r/open-invitation.json`

Requires React 18 or 19 and Tailwind CSS 4. No other runtime dependencies.

[Live demo](https://knock.codes/playground/open-invitation) · [Registry JSON](https://knock.codes/r/open-invitation.json) · [Source](https://knock.codes/source/open-invitation.tsx)
...
| heading | `ReactNode` | no | 'This look is for you.' | Overrides the default heading. |
| description | `ReactNode` | no | 'Continue when you are ready. Your application decides what opens.' | Overrides the default description. |
