...
Attempts have run out and the whole screen should become the pause, rather than a line of text under the field.

## Not for

Keeping the gate visible through a short wait. Use the Cooldown notice component inside the gate you already have.
...
npx shadcn@latest add @knock-codes/cooldown-screen
```

Direct URL:

`npx shadcn@latest add https://knock.codes/r/cooldown-screen.json`

Requires React 18 or 19 and Tailwind CSS 4. No other runtime dependencies.

[Live demo](https://knock.codes/playground/cooldown-screen) · [Registry JSON](https://knock.codes/r/cooldown-screen.json) · [Source](https://knock.codes/source/cooldown-screen.tsx)
...
| heading | `ReactNode` | no | 'Take a little pause.' | Overrides the default heading. |
| description | `ReactNode` | no | 'Your preview is still here. Give it a moment before your next try.' | Overrides the default description. |
...
- The countdown finishing never submits a code and never clears the cooldown. Re-enable entry from your own state.
- No attempts are counted here. The retry policy, and the deadline you pass in, are yours.
...
- heading: ReactNode; default 'Take a little pause.'
- description: ReactNode; default 'Your preview is still here. Give it a moment before your next try.'
...
Use it when: Attempts have run out and the whole screen should become the pause, rather than a line of text under the field.
Do not use it for: Keeping the gate visible through a short wait. Use the Cooldown notice component inside the gate you already have.
Common mistakes:
- The countdown finishing never submits a code and never clears the cooldown. Re-enable entry from your own state.
- No attempts are counted here. The retry policy, and the deadline you pass in, are yours.
...
import { CooldownScreen } from '@/components/knock/cooldown-screen';
