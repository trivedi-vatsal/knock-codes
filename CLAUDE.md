...
- `registry/react/registry.json` — shadcn-compatible registry entries, the hand-authored source. Build with `node scripts/build-registry.mjs`.
- `registry.json` (repo root) — generated, not hand-edited. A path-only copy of `registry/react/registry.json` for the GitHub `owner/repo/item` addressing shorthand, with same-registry `registryDependencies` rewritten to `trivedi-vatsal/knock-codes/<name>` so the CLI doesn't resolve bare names against ui.shadcn.com.
...
- **Registry build** — `node scripts/build-registry.mjs` (`pnpm registry:build`) runs `shadcn build` against `registry/react/registry.json` and writes `apps/web/public/r/react/*.json`, the files `shadcn add <url>` actually serves. It also regenerates the repo-root `registry.json` — a generated, path-only (no embedded `content`) copy of `registry/react/registry.json` with `name` overridden to `"knock-codes"` and same-registry deps rewritten to `trivedi-vatsal/knock-codes/<name>`, required for the GitHub `owner/repo/item` shorthand (`npx shadcn add trivedi-vatsal/knock-codes/<item>`), which only reads a registry.json at the repo root. Don't hand-edit `registry.json` at the root — it's regenerated by the build script; edit `registry/react/registry.json` instead. `include` can't be used to compose the two because `registry/react/registry.json`'s paths are repo-root-relative, while `include` resolves paths relative to the *included* file's own directory with no parent-traversal escape hatch. Items chain via `registryDependencies` (e.g. `knock-codes-hook` → `knock-codes-types` → `knock-codes-core`) — changing what a block imports usually means editing `registry.json` too, not just the `.tsx`.
...
- `pnpm registry:build` — regenerate `apps/web/public/r/react` and the root `registry.json` from `registry/react/registry.json`.
- `pnpm registry:check` — diffs the committed `apps/web/public/r/react` and root `registry.json` against a fresh `registry:build` output, byte-for-byte. Run this rather than trusting the build was re-run before commit — it's the actual guard against registry drift.

## Pitfalls

- **Registry drift** — editing a block/template's files or `registry/react/registry.json` without re-running `registry:build` leaves `apps/web/public/r/react` and the root `registry.json` stale. `registry:check` catches it.
